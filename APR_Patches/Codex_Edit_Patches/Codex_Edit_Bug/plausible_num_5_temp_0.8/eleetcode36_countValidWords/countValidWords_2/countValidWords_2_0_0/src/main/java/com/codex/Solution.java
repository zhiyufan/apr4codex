...
public class Solution {
    ...
    private static boolean isValid(String token) {
        ...
        boolean numberOfUpperCaseLettersCheck = false;
        ...
        for (int i=0; i<token.length(); i++) {
            ...
            if (isUpperCaseLetter(ch)) {
                numberOfUpperCaseLettersCheck = true;
            }
            if (!digitCheck) {
        ...
        if (numberOfUpperCaseLettersCheck) return false;
        if (punctuationCheck || digitCheck || hyphenCheck) {
            return false;
        }
        return true;
    }
