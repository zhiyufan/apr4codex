...
import java.util.ArrayList;
    ...
    private static boolean isValid(String token) {
        ...
        for (int i=0; i<token.length(); i++) {
            ...
            if (!hyphenCheck && !numberOfHyphensCheck) {
                if (ch == '-') {
                    ...
                    if ((i != 1 && !Character.isLowerCase(prevCharacter)) || !Character.isLowerCase(nextCharacter)) {
                        hyphenCheck = true;
                        continue;
                    }
        ...
        if (punctuationCheck || digitCheck || hyphenCheck || numberOfHyphensCheck) return false;
        return true;
    }
