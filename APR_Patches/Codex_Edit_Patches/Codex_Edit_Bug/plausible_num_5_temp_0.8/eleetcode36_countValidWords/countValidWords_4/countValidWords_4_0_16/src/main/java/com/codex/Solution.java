...
public class Solution {
    ...
    private static boolean isValid(String token) {
        ...
        if (token.length() > 1) {
                ...
                if (!Character.isAlphabetic(token.charAt(i)) && !Character.isDigit(token.charAt(i))) {
                    if (token.charAt(i) == '-') {
                        if (!Character.isAlphabetic(token.charAt(i - 1)) || !Character.isAlphabetic(token.charAt(i + 1))) &&
                            !Character.isDigit(token.charAt(i - 1)) || !Character.isDigit(token.charAt(i + 1))) {
                            return false;
                        }
                        hasHyphen = 1;
            ...
            if (Character.isAlphabetic(token.charAt(token.length() - 1)) && Character.isDigit(token.charAt(token.length() - 1))) {
                return true;
