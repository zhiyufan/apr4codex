...
public class Solution {
    ...
    private static boolean isValid(String token) {
        int previousCharIsAlphabetic = 0;
        ...
        if (token.length() > 1) {
            for (int i = 0; i < token.length() - 1; i++) {
                if (!Character.isAlphabetic(token.charAt(i))) {
                            return false;
                        } else if (!Character.isAlphabetic(token.charAt(i - 1))) {
                            return false;
                        } else if (!Character.isAlphabetic(token.charAt(i + 1))) {
                            return false;
                        } else if (!Character.isAlphabetic(token.charAt(i - 1)) && !Character.isAlphabetic(token.charAt(i + 1))) {
                    if (token.charAt(i) == '-') {
