...
public class Solution {
    ...
    public static int minMoves(int target, int maxDoubles) {
        ...
        while (target > 0) {
            ...
            for (int i = 1; i <= target; i++) {
                minMoves = Math.min(minMoves, Math.min(1 + minMoves(target - i, maxDoubles - 1), 1 + minMoves(target / 2, maxDoubles - 1)));
                maxMoves = Math.max(maxMoves, Math.min(1 + minMoves(target - i, maxDoubles - 1), 1 + minMoves(target / 2, maxDoubles - 1)));
            }
            if (minMoves == maxMoves) break;
            ans = minMoves;
            target--;
        }
        return ans;
