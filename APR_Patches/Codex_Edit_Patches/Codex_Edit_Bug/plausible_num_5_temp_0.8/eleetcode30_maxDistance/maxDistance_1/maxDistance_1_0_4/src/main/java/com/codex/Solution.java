...
public class Solution {
    ...
    public static int minDistance(int[] colors) {
        ...
        for (int i = 0; i < colors.length; i++) {
            distance = Math.min(distance, Math.abs(i - left.get(colors[i])));
            distance = Math.min(distance, Math.abs(i - right.get(colors[i])));
        }
