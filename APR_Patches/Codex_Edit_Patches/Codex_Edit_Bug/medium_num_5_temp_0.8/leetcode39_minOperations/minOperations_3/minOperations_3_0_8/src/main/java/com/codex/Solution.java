...
// This class shows DFS + Topological Sort solution for Sliding puzzle problem.
    public static int minOperations(int[][] grid, int x) {
        int N = grid.length;
        int M = grid[0].length;
        int[][] dp = new int[N][M]; // dp[i][j] stores number of moves required to move the zero to cell i,j
        int INF = Integer.MAX_VALUE;
         // Initialize dp values for all the cells
        for (int[] arr : dp){
            Arrays.fill(arr, -1);
        }
        dp[0][0] = 0;
