...
public class Solution {
    public static boolean checkMove(char[][] board, int rMove, int cMove, char color) {
        ...
        r = rMove;
        c = cMove;
        while (r < 7 && c > 0 && board[r + 1][c - 1] == (color == 'B' ? 'W' : 'B')) {
            r++;
            c--;
        }
        if (r < 6 && c > 1) {
            r--;
            c++;
            while (r > -1 && c < 8 && board[r][c] == color) {
                r--;
                c++;
            }
            if (r == -1 || c == 8 || board[r][c] != '.')
                return true;
        }
        r = rMove;
        c = cMove;
        while (r < 7 && c < 7 && board[r + 1][c + 1] == (color == 'B' ? 'W' : 'B')) {
            r++;
            c++;
        }
        if (r < 6 && c < 6) {
