...
public class Solution {
    public static int[][] findFarmland(int[][] farmland) {
        
        if (farmland == null || farmland.length == 0 || farmland[0] == null || farmland[0].length == 0) return new int[0][0];
        int m = farmland.length, n = farmland[0].length;
        int[][] result = new int[m * n][4];
        int iResult = 0;
        int[][] land = new int[m][n];
        for (int i = 0; i < m; i++) {
            land[i] = Arrays.copyOf(farmland[i], n);
        }
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (land[i][j] == 0) continue;
                result[iResult][0] = result[iResult][2] = i;
                result[iResult][1] = result[iResult][3] = j;
