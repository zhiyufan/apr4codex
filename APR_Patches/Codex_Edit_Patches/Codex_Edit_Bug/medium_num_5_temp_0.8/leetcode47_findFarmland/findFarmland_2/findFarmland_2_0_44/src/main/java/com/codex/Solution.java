    ...
    public static int[][] findFarmland(int[][] land) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                ...
                result[iResult][2] = i;
                result[iResult][3] = j;
                dfsFindFarmland(land, i, j, iResult, result, visited);
                iResult++;
            }
        }
        return Arrays.copyOf(result, iResult);
    }
    private static void dfsFindFarmland(int[][] land, int i, int j, int iResult, int[][] result, boolean[][] visited) {
        visited[i][j] = true;
        if (i > 0 && !visited[i - 1][j] && land[i - 1][j] == 1) {
            result[iResult][0] = Math.min(result[iResult][0], i - 1);
            dfsFindFarmland(land, i - 1, j, iResult, result, visited);
        }
        if (i < land.length - 1 && !visited[i + 1][j] && land[i + 1][j] == 1) {
            result[iResult][2] = Math.max(result[iResult][2], i + 1);
            dfsFindFarmland(land, i + 1, j, iResult, result, visited);
        }
        if (j > 0 && !visited[i][j - 1] && land[i][j - 1] == 1) {
            result[iResult][1] = Math.min(result[iResult][1], j - 1);
            dfsFindFarmland(land, i, j - 1, iResult, result, visited);
        }
        if (j < land[0].length - 1 && !visited[i][j + 1] && land[i][j + 1] == 1) {
