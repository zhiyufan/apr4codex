...
public class Solution {
    public static long maximumSubsequenceCount(String text, String pattern) {
        ...
        while (firstIndex < firstIndexes.size() && secondIndex < secondIndexes.size()) {
            if (first == -1 || (firstIndex < firstIndexes.size() && firstIndexes.get(firstIndex) < secondIndexes.get(secondIndex))) {
                first = firstIndexes.get(firstIndex);
                firstIndex++;
            } else if (second == -1 || (secondIndex < secondIndexes.size() && secondIndexes.get(secondIndex) < firstIndexes.get(firstIndex))) {
