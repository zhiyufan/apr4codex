...
public class Solution {
    public static long maximumSubsequenceCount(String text, String pattern) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (indexes == null) { // first occurrence of c
                indexes = new ArrayList<>(); // create a new empty list of indexes
            }
            indexes.add(i);
            charToIndexes.put(c, indexes);
        }

        if (!charToIndexes.containsKey(pattern.charAt(0))) {
            return 0;
        }

        if (!charToIndexes.containsKey(pattern.charAt(1))) {
            return 0;
        }
