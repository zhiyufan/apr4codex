...
public static String findDifferentBinaryString(String[] nums) {
    ...
    for (int i = 0; i < nums.length; i++) {
        if (i == 0 && nums[i].equals("0" + nums[i + 1])) {
            ...
            if (i == 0 && nums[i].equals("1" + nums[i + 1])) {
                return "0" + nums[i];
            } else if (i == nums.length - 1 && nums[i].equals(nums[i - 1] + "1")) {
                return nums[i] + "0";
            } else if (i != 0 && i != nums.length - 1 && nums[i].equals(nums[i - 1] + "1") && nums[i].equals("1" + nums[i + 1])) {
                return nums[i] + "0";
            }
        }
    }
