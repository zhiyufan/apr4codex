    ...
    public static int numberOfArrays(int[] differences, int lower, int upper) {
        ...
        for (int i = 1; i < differences.length + 1; i++) {
            if (differences[i - 1] == 0) {
                dp[i][1] = (dp[i][1] + dp[i - 1][0] * (upper - lower + 1) % mod) % mod;
                dp[i][0] = (dp[i][0] + dp[i - 1][0] * (upper - lower + 1) % mod) % mod;
                dp[i][1] = (dp[i][1] + dp[i - 1][1] * (upper - lower + 1) % mod + dp[i - 1][0] * (upper - lower + 1) % mod) % mod;
                ...
                dp[i][1] = (dp[i][1] + dp[i - 1][0] * (upper - lower) % mod + dp[i - 1][1] * (upper - lower) % mod) % mod;
