...
public class Solution {
    public static int nearestExit(char[][] maze, int[] entrance) {
        ...
        //Initially assume that it takes maximum steps to go to reach any point
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                dist[i][j] = Integer.MAX_VALUE;
            }
        }
        //Mark the distance at the entrance point as 0
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] dir : dirs) {
                ...
                //If the current point is not a wall
                ...
                //If the current distance is smaller then the current count
