...
public class Solution {
    public static int nearestExit(char[][] maze, int[] entrance) {
        ...
        HashSet<Integer> notMove = new HashSet<>();
        notMove.add(entrance[0] * 10000 + entrance[1]);
        int[] dx = {-1, 0, 1, 0};
        int[] dy = {0, 1, 0, -1};
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                while (isValid(maze, nextX, nextY) && maze[nextX][nextY] == '.' && !notMove.contains(nextX * 10000 + nextY)) {
                    nextX += dx[i];
                    nextY += dy[i];
                    step++;
                }
