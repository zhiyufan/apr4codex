...
public class Solution {
    public static int nearestExit(char[][] maze, int[] entrance) {
        ...
        for (int i = 0; i < m; i++) {
            if (maze[i][0] == '.' && distTo[i][0] != Integer.MAX_VALUE)
                min = Math.min(distTo[i][0], min);
            if (maze[i][n - 1] == '.' && distTo[i][n - 1] != Integer.MAX_VALUE)
                min = Math.min(distTo[i][n - 1], min);
        }
        for (int j = 0; j < n; j++) {
            if (maze[0][j] == '.' && distTo[0][j] != Integer.MAX_VALUE)
                min = Math.min(distTo[0][j], min);
            if (maze[m - 1][j] == '.' && distTo[m - 1][j] != Integer.MAX_VALUE)
