package com.codex;

import java.util.*;

public class Solution {
// a function that returns the max sum of a matrix's elements

#include <bits/stdc++.h>

using namespace std;

long maxMatrixSum(int** matrix, int rows, int cols) {
        

        //int rows = matrix.length;
        //int cols = matrix[0].length;

        int** matrixSum = new int*[rows];
	for (int i = 0; i < rows; ++i) {
		matrixSum[i] = new int[cols];
	}

        matrixSum[0][0] = matrix[0][0];

        for(int i=1;i<rows;i++) {
            matrixSum[i][0] = matrixSum[i-1][0] + matrix[i][0];
        }

        for(int j=1;j<cols;j++) {
            matrixSum[0][j] = matrixSum[0][j-1] + matrix[0][j];
        }

        for(int i=1;i<rows;i++) {
            for(int j=1;j<cols;j++) {
                matrixSum[i][j] = matrix[i][j] + max(matrixSum[i-1][j],matrixSum[i][j-1]);
            }
        }

        long maxSum = Long.MIN_VALUE;
        for(int i=0;i<rows;i++) {
            for(int j=0;j<cols;j++) {
                if(maxSum<matrixSum[i][j]) {
                    maxSum = matrixSum[i][j];
                }
            }
        }

        return maxSum;
    }
}