...
public class Solution {
    public static long maxMatrixSum(int[][] matrix) {
        ...
        for (int ii = 0; ii < matrix.length; ii++) {
            for (int jj = 0; jj < matrix[0].length; jj++) {
                max = Math.max(max, maxMatrixSum(matrix, ii, jj));
            }
        }
        return max;
    }

    private static long maxMatrixSum(int[][] matrix, int i, int j) {
        long rowSum = 0;
        long colSum = 0;
        ...
        long max = rowSum + colSum;
        for (int ii = 0; ii < matrix.length; ii++) {
            if (ii != i) {
                max = Math.max(max, rowSum + maxMatrixSum(matrix, ii, j));
            }
        }
        for (int jj = 0; jj < matrix[0].length; jj++) {
            if (jj != j) {
                max = Math.max(max, colSum + maxMatrixSum(matrix, i, jj));
            }
        }
        return max;
    }
    ...
    private static long maxMatrixSumCol(int[][] matrix, int i, int j) {
        ...
        for (int ii = 0; ii < matrix.length; ii++) {
            for (int jj = 0; jj < matrix[0].length; jj++) {
                max = Math.max(max, maxMatrixSumCol(matrix, i, j, ii, jj));
        }
    ...
    private static long maxMatrixSumCol(int[][] matrix, int i, int j, int ii, int jj) {
        long max = 0;
        for (int ii = 0; ii < matrix.length; ii++) {
            for (int jj = 0; jj < matrix[0].length; jj++) {
                long colSum = 0;
