...
public class Solution {
    ...
    if (matrix[i][j] == 1) {
                        if (reachability.get(start).get(end) != null) {
                            continue;
                        }
                        reachability.get(start).add(end);
                        if (reachability.get(end) != null && reachability.get(end).get(start) != null) {
                            continue;
                        }
                        if (reachability.get(end) == null) {
                            reachability.put(end, new LinkedList<>());
                        }
                        reachability.get(end).add(start);
                    }
