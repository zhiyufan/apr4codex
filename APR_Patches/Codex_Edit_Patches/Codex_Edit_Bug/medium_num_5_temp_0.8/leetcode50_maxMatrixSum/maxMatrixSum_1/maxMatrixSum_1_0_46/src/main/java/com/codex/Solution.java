...
public class Solution {
    public static long maxMatrixSum(int[][] matrix) {
		...
		// Sum all the rows to find the max value
        int maxSum = 0;
        ...
        // Calculate the sum of all the rows and overwrite the maxSum if it is greater
        for (int i = 0; i < m1.length; i++) {
            int sum = 0;
            for (int j = 0; j < m1[0].length; j++) {
                sum += m1[i][j];
        ...
        // Calculate the sum of all the columns and overwrite the maxSum if it is greater
