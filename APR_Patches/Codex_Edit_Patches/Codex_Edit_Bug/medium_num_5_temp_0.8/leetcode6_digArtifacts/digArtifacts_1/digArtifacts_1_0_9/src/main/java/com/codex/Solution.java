...
public class Solution {
    public static int digArtifacts(int n, int[][] artifacts, int[][] dig) {
        ...
        Set<Integer> digIndexSet = new HashSet<>();
        
        for (int[] digpos : dig) {
            int r = digpos[0];
            int c = digpos[1];
            int index = r * n + c;
            digIndexSet.add(index);
        }
        ...
        for (int[] digpos : dig) {
            ...
            for (int[] artifact : set) {                
                int nonDigCount = 0;
                for (int dr = artifact[0]; dr <= artifact[2]; dr++) {
                    for (int dc = artifact[1]; dc <= artifact[3]; dc++) {
                        int index = dr * n + dc;
                        if (!digIndexSet.contains(index)) {
                            nonDigCount++;
                        }
                    }
                }
