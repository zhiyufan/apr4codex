...
public class Solution {
    public static int digArtifacts(int n, int[][] artifacts, int[][] dig) {
        ...
        for (int i = 1; i <= m; i++) {
            ...
            rng[i] = Math.max(rows[i], cols[i]) * n * n;
        }

        Arrays.sort(idx, (i, j) -> rng[dig[j][0] * n + dig[j][1]] - rng[dig[i][0] * n + dig[i][1]] + (n - dig[j][0]) + (n - dig[j][1]) - (n - dig[i][0]) + (n - dig[i][1]));

        for (int i = 0; i < d; i++) {
            int art = dig[idx[i]][0] * n + dig[idx[i]][1] + 1;
            if (rows[art] == 0) {
                continue;
            }
            count++;
