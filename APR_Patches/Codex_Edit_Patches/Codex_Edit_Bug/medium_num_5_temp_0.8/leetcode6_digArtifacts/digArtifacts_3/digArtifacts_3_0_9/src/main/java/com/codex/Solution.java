...
public static int digArtifacts(int n, int[][] artifacts, int[][] dig) {
        ...
        for(int i = 0; i < d; i++) {
            int rpos = dig[i][0] * n + dig[i][1];
            int dist = rows[rpos] * cols[rpos];
            rngs[rpos] = dist == 0 ? 0 : dist;
            idx[i] = i;
        }

        Arrays.sort(idx, (i, j) -> rngs[dig[j][0] * n + dig[j][1]] - rngs[dig[i][0] * n + dig[i][1]]);
        int count = 0;

        for(int i = 0; i < d; i++) {
            int art = dig[idx[i]][0] * n + dig[idx[i]][1];
            if(rows[art] == 0) continue;
            count++;
            rows[art] = 0;
            cols[art] = 0;
