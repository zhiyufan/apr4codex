    ...
    for (int i = 1; i <= n - 1; i++) {
        ...
        for (int j = i + 1; j <= n; j++) {
            dp[cur][j] = Math.min(
                    ...
                    dp[cur][j - 1] + (colors.charAt(j - 1) == 'A' ? 1 : 0)
            );
        }
    }
...
public class WinnerOfGame {
    ...
    public static boolean winnerOfGame(String s) {
        ...
        int n = s.length();
        int[][] dp = new int[2][n + 1]; //0:A, 1:B

        for (int i = 1; i <= n; i++) {
            dp[1][i] = dp[1][i - 1] + (s.charAt(i - 1) == 'B' ? 1 : 0);
        }

        for (int i = 1; i <= n; i++) {
            int cur = i % 2, pre = (i - 1) % 2;
            dp[cur][i] = dp[pre][i] + (s.charAt(i - 1) == 'A' ? 1 : 0);
            for (int j = i + 1; j <= n; j++) {
                dp[cur][j] = Math.min(dp[pre][j - 1] + (s.charAt(j - 1) == 'B' ? 1 : 0),
                        dp[cur][j - 1] + (s.charAt(j - 1) == 'A' ? 1 : 0)
                );
            }
        }
