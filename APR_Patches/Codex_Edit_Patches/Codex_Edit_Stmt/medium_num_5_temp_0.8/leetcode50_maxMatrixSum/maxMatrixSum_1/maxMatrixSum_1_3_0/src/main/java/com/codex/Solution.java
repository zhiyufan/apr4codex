...
public class Solution {
    public static long maxMatrixSum(int[][] matrix) {
        ...
        // 1. вычислить сумму матрицы
        // 2. записать какие колонки в какие строчки менять
        // 3. рассчитать отрицательные суммы от строчек
        // 4. рассчитать отрицательные суммы от колонок
        // 5. сложить получившиеся суммы
        ...
        for (int rowToSwap : rowsToSwap) {
            sum += -2 * matrix[rowToSwap][rowToSwap];
        }
        for (int colToSwap : colsToSwap) {
            sum += -2 * matrix[colToSwap][colToSwap];
        }
