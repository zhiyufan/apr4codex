...
public class Solution {
    public static int nearestExit(char[][] maze, int[] entrance) {
        ...
        for (int i = 0; i < maze.length; i++) {
            if (maze[i][0] == '.' && dist[i][0] < res) { // left
                res = dist[i][0]; // left
            }
            if (maze[i][maze[0].length - 1] == '.' && dist[i][maze[0].length - 1] < res) { // right
                res = dist[i][maze[0].length - 1]; // right
            }
        }
        for (int i = 0; i < maze[0].length; i++) {
            if (maze[0][i] == '.' && dist[0][i] < res) { // up
                res = dist[0][i]; // up
            }
            if (maze[maze.length - 1][i] == '.' && dist[maze.length - 1][i] < res) { // down
                res = dist[maze.length - 1][i]; // down
            }
        }
