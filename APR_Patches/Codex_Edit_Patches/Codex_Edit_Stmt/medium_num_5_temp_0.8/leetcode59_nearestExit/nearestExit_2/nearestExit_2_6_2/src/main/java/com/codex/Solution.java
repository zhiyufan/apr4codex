...
public class Solution {
    public static int nearestExit(char[][] maze, int[] entrance) {
        ...
        for (int i = 0; i < maze.length; i++) {
            if (maze[i][0] == '.' && dist[i][0] > 0) {
                res = Math.min(res, dist[i][0]);
            }
            if (maze[i][maze[0].length - 1] == '.' && dist[i][maze[0].length - 1] > 0) {
                res = Math.min(res, dist[i][maze[0].length - 1]);
            }
        }
        for (int i = 0; i < maze[0].length; i++) {
            if (maze[0][i] == '.' && dist[0][i] > 0) {
                res = Math.min(res, dist[0][i]);
            }
            if (maze[maze.length - 1][i] == '.' && dist[maze.length - 1][i] > 0) {
                res = Math.min(res, dist[maze.length - 1][i]);
            }
        }
