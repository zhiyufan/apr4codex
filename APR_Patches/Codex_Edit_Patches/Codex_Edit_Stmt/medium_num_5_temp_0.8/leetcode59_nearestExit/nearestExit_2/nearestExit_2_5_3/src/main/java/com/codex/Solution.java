...
public class Solution {
    public static int nearestExit(char[][] maze, int[] entrance) {
        ...
        for (int i = 0; i < maze.length; i++) {
            ...
            if (maze[i][maze[0].length - 1] == '.' && dist[i][maze[0].length - 1] < res && dist[i][maze[0].length - 1] != Integer.MAX_VALUE) {
        ...
        for (int i = 0; i < maze[0].length; i++) {
            ...
            if (maze[maze.length - 1][i] == '.' && dist[maze.length - 1][i] < res && dist[maze.length - 1][i] != Integer.MAX_VALUE ) {
