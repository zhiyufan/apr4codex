...
public class Solution {
    public static List<Integer> maxScoreIndices(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            int zeroLeft = countNumberOfZeros(nums, 0, i - 1);
            int oneRight = countNumberOfOnes(nums, i, nums.length - 1);
            int score = zeroLeft + oneRight;
            if (score > maxScore) {
                maxScore = score;
                result.clear();
                result.add(i);
    ...
    private static int countNumberOfOnes(int[] nums, int left, int right) {
        int result = 0;
    ...
    private static int countNumberOfZeros(int[] nums, int left, int right) {
        int result = 0;
        for (int i = left; i <= right; i++) {
            if (nums[i] == 0) {
                result++;
