...
public class Solution {
    ...
    /**
     * 找到面积最大的农田，返回[上, 左, 下, 右]。未找到则返回[]
     * @param land
     * @return
     */
    public static int[] findMaxFarmland(int[][] land) {

        int[][] farmland = findFarmland(land);
        int[] result = new int[4];
        int maxArea = Integer.MIN_VALUE;
        for (int i = 0; i < farmland.length; i++) {
            int area = (farmland[i][2] - farmland[i][0] + 1) * (farmland[i][3] - farmland[i][1] + 1);
            if (area > maxArea) {
                maxArea = area;
                result = farmland[i];
            }
