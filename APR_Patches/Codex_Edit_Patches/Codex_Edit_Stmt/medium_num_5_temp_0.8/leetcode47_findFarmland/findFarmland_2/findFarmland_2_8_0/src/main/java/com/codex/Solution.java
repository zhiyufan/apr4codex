    ...
    public static int[][] findFarmland(int[][] land) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                ...
                result[iResult][2] = i;
                result[iResult][3] = j;
                dfsFindFarmland(land, i, j, iResult, result, m, n);
                iResult++;
            }
        }
        return Arrays.copyOf(result, iResult);
    }
    private static void dfsFindFarmland(int[][] land, int i, int j, int iResult, int[][] result, int m, int n) {
        land[i][j] = 0;
        if (i > 0 && land[i - 1][j] == 1) {
        ...
        if (i < m - 1 && land[i + 1][j] == 1) {
            ...
            dfsFindFarmland(land, i + 1, j, iResult, result, m, n);
        }
        if (j > 0 && land[i][j - 1] == 1) {
        ...
        if (j < n - 1 && land[i][j + 1] == 1) {
            ...
            dfsFindFarmland(land, i, j + 1, iResult, result, m, n);
        }
        ...
        if (i > 0 && j < n - 1 && land[i - 1][j + 1] == 1) {
            result[iResult][0] = Math.min(result[iResult][0], i - 1);
            result[iResult][3] = Math.max(result[iResult][3], j + 1);
            dfsFindFarmland(land, i - 1, j + 1, iResult, result, m, n);
        }
        if (i < m - 1 && j > 0 && land[i + 1][j - 1] == 1) {
            result[iResult][2] = Math.max(result[iResult][2], i + 1);
            result[iResult][1] = Math.min(result[iResult][1], j - 1);
            dfsFindFarmland(land, i + 1, j - 1, iResult, result, m, n);
        }
        if (i < m - 1 && j < n - 1 && land[i + 1][j + 1] == 1) {
            result[iResult][2] = Math.max(result[iResult][2], i + 1);
            result[iResult][3] = Math.max(result[iResult][3], j + 1);
            dfsFindFarmland(land, i + 1, j + 1, iResult, result, m, n);
        }
