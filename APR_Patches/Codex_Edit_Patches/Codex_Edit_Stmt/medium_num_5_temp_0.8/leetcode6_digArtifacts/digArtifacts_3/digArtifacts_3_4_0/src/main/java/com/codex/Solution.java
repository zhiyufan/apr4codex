...
public class Solution {
    public static int digArtifacts(int n, int[][] artifacts, int[][] dig) {
        ...
        for (int i = 0; i < m; i++) {
            rows[i] = Math.max(artifacts[i][0], artifacts[i][2]) - Math.min(artifacts[i][0], artifacts[i][2]);
            cols[i] = Math.max(artifacts[i][1], artifacts[i][3]) - Math.min(artifacts[i][1], artifacts[i][3]);
