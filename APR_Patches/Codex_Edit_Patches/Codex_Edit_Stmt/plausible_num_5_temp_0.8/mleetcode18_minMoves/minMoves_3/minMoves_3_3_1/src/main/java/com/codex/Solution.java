...
public class Solution {
    ...
    private static int minMoves(int target, int maxDoubles, Map<String, Integer> memo) {
        if (target == 0) return 0;
        if (maxDoubles == 0) return target;
        if (memo.containsKey(target + " " + maxDoubles)) return memo.get(target + " " + maxDoubles);
        ...
        memo.put(target + " " + maxDoubles, minMoves);
        return minMoves;
