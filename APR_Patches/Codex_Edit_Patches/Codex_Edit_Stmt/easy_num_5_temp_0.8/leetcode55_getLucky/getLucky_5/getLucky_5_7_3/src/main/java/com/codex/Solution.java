...
public class Solution {
    public static int getLucky(String s, int k) {
        ...
        if (k == 1) {
            ...
            int sum = 0;
            int c = convert;
            while (c > 0) { // FIXME c is never greater than 0, so this loop can never be entered
                c = c / 10;
                ++digit;
            }

            while (convert > 0) { // FIXME convert is never greater than 0, so this loop can never be entered
                int r = convert % 10;
                sum = sum + r;
                convert = convert / 10;
            }
