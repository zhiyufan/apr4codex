...
public class Solution {
    public static int getLucky(String s, int k) {
        ...
        } else if (convert > 9 && k == 1) {
            transformation = convert;
        } else {
            int digit = s.length(); //Determine the number of digits
            int sum = 0;
            int c = convert;
            while (c > 0) { //Determine the number of digits
                c = c / 10;
                ++digit;
            }

            while (convert > 0) { //Sum all digits in convert
                int r = convert % 10;
                sum = sum + r;
                convert = convert / 10;
            }
