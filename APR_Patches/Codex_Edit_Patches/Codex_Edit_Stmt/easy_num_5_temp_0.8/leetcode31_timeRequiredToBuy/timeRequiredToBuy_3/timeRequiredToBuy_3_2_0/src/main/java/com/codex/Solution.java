...
public class Solution {
    public static int timeRequiredToBuy(int[] tickets, int k) {
        ...
        for (int ticket : tickets) {
            maxTicketsNeeded = Math.max(maxTicketsNeeded, ticket);
        }

        if (maxTicketsNeeded > k) return maxTicketsNeeded * 2 - 1;

        int currentTicketsNeeded = 0;

        for (int ticket : tickets) if (ticket > currentTicketsNeeded) {
            currentTicketsNeeded = ticket;
        }
