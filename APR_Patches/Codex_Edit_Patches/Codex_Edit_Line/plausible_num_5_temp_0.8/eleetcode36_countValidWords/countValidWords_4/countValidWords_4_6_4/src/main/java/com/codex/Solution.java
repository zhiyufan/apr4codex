...
public class Solution {
    ...
    private static boolean isValid(String token) {
        ...
        if (token.length() > 1) {
            ...
            if (Character.isAlphabetic(token.charAt(token.length() - 1))) {
                if (isArithmetic(token)) return false;
                return true;
    ...
    private static boolean isArithmetic(String token) {
    	String[] arithm = {"plus", "minus", "times", "over", "equals", "equal", "equal"};
    	for (String ar : arithm)
    		if (token.equals(ar)) return true;
    	return false;
    }
