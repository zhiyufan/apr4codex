        ...
        for (int i = 0; i < land.length; i++) {
            for (int j = 0; j < land[i].length; j++) {
                ...
                while (!queue.isEmpty()) {
                    ...
                    if (curr[0] - 1 >= 0 && land[curr[0] - 1][curr[1]] == 1 && !visited[curr[0] - 1][curr[1]]) {
                        queue.offer(new Integer[]{curr[0] - 1, curr[1]});
                    }
                    if (curr[1] - 1 >= 0 && land[curr[0]][curr[1] - 1] == 1 && !visited[curr[0]][curr[1] - 1]) {
                        queue.offer(new Integer[]{curr[0], curr[1] - 1});
                    }
                    if (curr[0] + 1 < land.length && land[curr[0] + 1][curr[1]] == 1 && !visited[curr[0] + 1][curr[1]]) {
                        queue.offer(new Integer[]{curr[0] + 1, curr[1]});
                    }
                    if (curr[1] + 1 < land[curr[0]].length && land[curr[0]][curr[1] + 1] == 1 && !visited[curr[0]][curr[1] + 1]) {
                        queue.offer(new Integer[]{curr[0], curr[1] + 1});
                    }
                }
