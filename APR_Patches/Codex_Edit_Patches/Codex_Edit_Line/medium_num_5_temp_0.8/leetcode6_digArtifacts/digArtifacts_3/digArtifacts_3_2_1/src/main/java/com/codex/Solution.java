...
public class Solution {
    public static int digArtifacts(int n, int[][] artifacts, List<List<Integer>> dig) {
        ...
        Arrays.sort(idx, (i, j) -> rng[dig.get(j).get(0) * n + dig.get(j).get(1)] - rng[dig.get(i).get(0) * n + dig.get(i).get(1)]);

        for (int i = 0; i < d; i++) {
            int art = dig.get(idx[i]).get(0) * n + dig.get(idx[i]).get(1);
            if (rows[art] == 0) {
                continue;
            }
            count++;
