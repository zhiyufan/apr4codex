...
public class Solution {
    public static int digArtifacts(int n, int[][] artifacts, int[][] dig) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0; j < digRow.length; j++) if (digRow[j] >= row1 && digRow[j] <= row2) rowCovered = true;
            for (int j = 0; j < digCol.length; j++) if (digCol[j] >= col1 && digCol[j] <= col2) colCovered = true;
