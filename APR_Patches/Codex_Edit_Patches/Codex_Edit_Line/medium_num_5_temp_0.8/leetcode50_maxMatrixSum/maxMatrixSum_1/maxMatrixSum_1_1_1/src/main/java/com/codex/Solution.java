...
public class Solution {
    ...
    private static void swap(int[][] matrix, List<Integer> rowsToSwap, List<Integer> colsToSwap) {
        for (int rowToSwap : rowsToSwap) {
            int temp = matrix[rowToSwap][0];
            matrix[rowToSwap][0] = matrix[rowToSwap][1];
            matrix[rowToSwap][1] = temp;
        }
        for (int colToSwap : colsToSwap) {
            int temp = matrix[0][colToSwap];
            matrix[0][colToSwap] = matrix[1][colToSwap];
            matrix[1][colToSwap] = temp;
        }
    }
}