...
public class Solution {
    ...
    private static long maxMatrixSumCol(int[][] matrix, int i, int j) { // Change line 48: jj to j
        long max = 0;
        for (int ii = 0; ii < matrix.length; ii++) {
            for (int jj = 0; jj < matrix[0].length; jj++) { // Change line 51: j to jj
                long colSum = 0;
                long rowSum = 0;
                for (int k = 0; k < matrix.length; k++) {
                    rowSum += matrix[ii][k];
                    colSum += matrix[k][jj]; // Change line 58: j to jj
                }
                if (ii == i) {
