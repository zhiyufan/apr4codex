    ...
    public static int minimumBuckets(String street) {
        ...
        while(i < streetChars.length){
            if(streetChars[i] == 'H'){
                ...
                else if(lastBucketPlaced && streetChars[i - 1] != '.'){//this check is wrong you need to check if the i-1th element is '.', If you can't place it and the previous element is not '.' then return -1
                    return -1;
                }
                else{
                    lastBucketPlaced = true;
                    i++;
                    bucketsCount++;//you don't need to increment the buckets count here.
                }
            }
            else{
                return -1;
