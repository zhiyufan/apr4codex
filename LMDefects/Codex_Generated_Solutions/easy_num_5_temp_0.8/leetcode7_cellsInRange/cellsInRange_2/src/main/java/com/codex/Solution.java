...
public class Solution {
    public static List<String> cellsInRange(String s) {
        ...
        for (int i = cellToNum(col1); i <= cellToNum(col2); i++) {
            for (int j = row1; j <= row2; j++) {
                res.add(numToCell(i) + j);
            }
        }
    ...
    private static int cellToNum(String cell) {
        int res = 0;
        for (int i = 0; i < cell.length(); i++) {
            res = res * 26 + (cell.charAt(i) - 'A' + 1);
        }
        return res;
    }

    private static String numToCell(int num) {
